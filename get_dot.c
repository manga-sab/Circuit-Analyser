    ...
    typedef struct node
    {
        ...
        typedef struct list
        {
            ...
            void print_node(node *n) 
            {
                ...
                long int max_hull_finder(int max_in1)
                {
                long dist=0;
                for (int i=0;i<hull1[max_in1].size()-2;i++)
                    {
                    int j=i+1;
                    
                        if (dist2(hull1[max_in1][i],hull1[max_in1][j])>dist)                   
                        dist=dist2(hull1[max_in1][i],hull1[max_in1][j]);
                    
                    }
                    return dist;
               } 
               /*
               void separater_based_on_feature_points()
               {
               Mat type1;
               Mat tester=fin.clone();
                          for (int i=0;i<ftr_pts_cntr-1;i++)
                        {
                        printf("originx-%d,originy-%d,height-%d,width-%d\n",ftr_pts1[i].pt.x,ftr_pts1[i+1].pt.y,abs(ftr_pts1[i+1].pt.y-ftr_pts1[i].pt.y),abs(ftr_pts1[i+1].pt.x-ftr_pts1[i].pt.x));
                        if (ftr_pts1[i].pt.y<=ftr_pts1[i+1].pt.y)
                                                circle(tester,Point(ftr_pts1[i+1].pt.x,ftr_pts1[i].pt.y),5,Scalar(255,0,0),-1);
                                                else
                                                circle(tester,Point(ftr_pts1[i].pt.x,ftr_pts1[i+1].pt.y),5,Scalar(255,0,0),-1);                                                
                                                circle(tester,ftr_pts1[i+1].pt,5,Scalar(255,0,0),-1);
                                                imshow("tester",tester);
                        cvWaitKey(0);
                        tester=fin.clone();
                        Rect roi;
                                                if (ftr_pts1[i].pt.y<=ftr_pts1[i+1].pt.y)
                      	       roi=Rect(ftr_pts1[i+1].pt.x,ftr_pts1[i].pt.y,abs(ftr_pts1[i+1].pt.x-ftr_pts1[i].pt.x),abs(ftr_pts1[i+1].pt.y-ftr_pts1[i].pt.y));
                      	       else
                      	      roi=Rect(ftr_pts1[i].pt.x,ftr_pts1[i+1].pt.y,abs(ftr_pts1[i+1].pt.x-ftr_pts1[i].pt.x),abs(ftr_pts1[i+1].pt.y-ftr_pts1[i].pt.y));                      	       
                      	       type1=fin(roi);
               ...
               */
                void join(int max_in1)
                {
                    ftr_pts_cntr=0;
                    printf("here2\n");
                    long int join_eps=max_hull_finder(max_in1);
                    join_eps/=3;
                    printf("Join eps-%ld\n",join_eps);
                    Mat join_temp;
                    join_temp = Mat::zeros(fin.size(), CV_8UC3 );
                    for (int i=0;i<hull1[max_in1].size()-1;i++)
                    {
                        int j=i+1;
                        circle(join_temp,hull1[max_in1][i],5,Scalar(255,0,0),-1);
                        circle(join_temp,hull1[max_in1][j],5,Scalar(0,0,255),-1);
   //                   imshow("join_temp",join_temp);
//                    cvWaitKey(0);
                        ...
                        if (dist2(hull1[max_in1][i],hull1[max_in1][j])<join_eps)
                        {
                            ...
                            else
                            {
                                ...
                                printf("distance-%d\n",dist2(hull1[max_in1][i],hull1[max_in1][j]));
                                circle(join_temp,ftr_pts1[ftr_pts_cntr].pt,5,Scalar(255,0,255),-1);
	                      //imshow("join_temp",join_temp);
	                   // cvWaitKey(0);
                                if (ftr_pts_cntr!=0)
                                {
                                ...
                                ftr_pts_cntr++;
                                                ...
                                                join_temp=Scalar(0,0,0);
                        }
                        ftr_pts1[ftr_pts_cntr-1].next=0;
                        for (int i=0;i<ftr_pts_cntr;i++)
                        {
                        circle(join_temp,ftr_pts1[i].pt,5,Scalar(0,255,255),-1);
	                      imshow("join_temp",join_temp);
	              //      cvWaitKey(0);                        
                        }
                        ...
                        cvWaitKey(0);
                  //      separater_based_on_feature_points();
                    }
                    void elm_rep()
                    {
                        int eps=200;
